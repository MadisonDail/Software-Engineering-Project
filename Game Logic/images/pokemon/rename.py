...
# Mapping of Pokédex numbers to Pokémon names
pokemon_names = {
...
def rename_files_in_folder(folder_path):
    ...
    for filename in files:
        # Assume the file name is a number with an extension, like '1.jpg'
        number_part, extension = os.path.splitext(filename)
        
        if number_part.isdigit():  # Check if the filename (without extension) is a digit
            number = int(number_part)
            
            # Find the Pokémon name for the corresponding number
            if number in pokemon_names:
                new_name = f"{pokemon_names[number].capitalize()}{extension}"
                ...
                print(f"No Pokémon found for number {number}")
        else:
            print(f"Filename '{filename}' does not represent a number.")

# Usage
folder_path = '/Users/Madison/Downloads/front-2'
