...
cursor.execute("SELECT * FROM pokemon WHERE id IN (2,4,7)")
...
while row is not None:
    ...
    pokemon = pmon(*row)
    userParty.append(pokemon)
    # Fetch the next row
    row = cursor.fetchone()

cursor.execute("SELECT * FROM pokemon WHERE id IN (1,3,5)")
...
print("User Party is: ", userParty)
print("Enemy Party is: ", enemyParty)
